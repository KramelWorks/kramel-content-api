...
export class ActivatedProjectUseCase implements IUseCase<string,ProjectDto>{
    constructor(
    ...
    public async execute(input: string, options?: AppOptions): Promise<ApiResult<ProjectDto>> {
        try {
            ...
            const activated=await this.repository.update(input,project);    

            if(!activated) {return ApiResult.fail<ProjectDto>(500,AppError.SERVER_ERROR.message);} 

            const result=this.mapper.toDto(activated);
