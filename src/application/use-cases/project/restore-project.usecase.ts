...
export class RestoreProjectUseCase implements IUseCase<string,ProjectDto>{
    constructor(
    ...
    public async execute(input: string, options?: AppOptions): Promise<ApiResult<ProjectDto>> {
        try {
            ...
            const restored=await this.repository.update(input,project);    

            if(!restored) {return ApiResult.fail<ProjectDto>(500,AppError.SERVER_ERROR.message);} 
