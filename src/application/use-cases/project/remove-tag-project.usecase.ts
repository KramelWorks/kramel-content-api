...
export class RemoveTagProjectUseCase implements IUseCase<TagDto,ProjectDto>{
    constructor(
    ...
    public async execute(input: TagDto, options?: AppOptions): Promise<ApiResult<ProjectDto>> {
        try {
            ...
            project.removeTag(input.tag);

            const untagged=await this.repository.update(input.id,project);    

            if(!untagged) {return ApiResult.fail<ProjectDto>(500,AppError.SERVER_ERROR.message);} 

            const result=this.mapper.toDto(untagged);
