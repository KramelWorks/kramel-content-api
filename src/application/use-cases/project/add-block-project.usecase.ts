...
export class AddBlockProjectUseCase implements IUseCase<AddRemoveBlockDto,ProjectDto>{
    ...
    public async execute(input: AddRemoveBlockDto, options?: AppOptions): Promise<ApiResult<ProjectDto>> {
        try {
            ...
            block.setProject(input.id);

            const addBlock=await this.blockRepository.update(input.blockId,block);    

            if(!addBlock) {return ApiResult.fail<ProjectDto>(500,AppError.SERVER_ERROR.message);} 
