...
export class ActivatedBlockUseCase implements IUseCase<string,BlockDto>{
    constructor(
    ...
    public async execute(input: string, options?: AppOptions): Promise<ApiResult<BlockDto>> {
        try {
            ...
            const activated=await this.repository.update(input,block);    

            if(!activated) {return ApiResult.fail<BlockDto>(500,AppError.SERVER_ERROR.message);} 
