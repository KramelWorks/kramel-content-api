...
export class FindByVersionBlockUseCase implements IUseCase<FindVersion,BlockDto>{
    constructor(
    ...
    public async execute(input: FindVersion, options?: AppOptions): Promise<ApiResult<BlockDto>> {
        try {

            const block=await this.repository.findByVersion(input.slug,input.version,options);
