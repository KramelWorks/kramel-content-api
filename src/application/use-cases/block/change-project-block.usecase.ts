...
import type { BlockProjectDto } from '../../dtos/block/block-project.dto';

export class ChangeProjectBlockUseCase implements IUseCase<BlockProjectDto,BlockDto>{
    constructor(
    ...
    public async execute(input: BlockProjectDto, options?: AppOptions): Promise<ApiResult<BlockDto>> {
        try {

            const block=await this.repository.findById(input.id,options);

            if(!block){return ApiResult.fail<BlockDto>(404,AppError.NOT_FOUND.message);}

            if(input.newProject==block.projectId){return ApiResult.fail<BlockDto>(403,AppError.FORBIDDEN.message);}

            block.setProject(input.newProject);

            const projectChanged=await this.repository.update(input.id,block);

            if(!projectChanged){return ApiResult.fail<BlockDto>(500,AppError.UNKNOWN_ERROR.message);}
