...
export class RestoreBlockUseCase implements IUseCase<string,BlockDto>{
    constructor(
    ...
    public async execute(input: string, options?: AppOptions): Promise<ApiResult<BlockDto>> {
        try {
            ...
            const restored=await this.repository.update(input,block);    

            if(!restored) {return ApiResult.fail<BlockDto>(500,AppError.SERVER_ERROR.message);} 
